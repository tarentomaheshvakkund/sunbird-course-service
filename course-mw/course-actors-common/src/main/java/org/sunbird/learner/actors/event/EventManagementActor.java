...
public class EventManagementActor extends BaseActor {
  ...
  private RedisCacheUtil cacheUtil = new RedisCacheUtil();
  ...
  private void getUserEnrolEventSummary(Request request) {
    ...
    try {
      ...
      cacheUserEnrolmentSummary(request, userId, response);
      sender().tell(response, self());
    } catch (Exception e) {
      logger.error(
          request.getRequestContext(), "Exception in enrolment list for user: " + userId, e);
  ...
  private Map<String, Object> getUserEnrolmentEventInfo(
    ...
    for (Map<String, Object> eventDetails : finalEnrolment) {
      ...
      if (eventStatus != null && eventStatus == 2) {
        List<Map<String, Object>> certificatesIssued =
            (List<Map<String, Object>>) eventDetails.get(JsonKey.ISSUED_CERTIFICATES);
      ...
      try {
        ...
        if (lrcProgressDetailsJson != null
            && lrcProgressDetailsJson.hasNonNull(JsonKey.DURATION)
            && isCertificateExist) {
  ...
  private void cacheUserEnrolmentSummary(Request request, String userId, Response response) {
    try {
      String key = JsonKey.USER_EVENT_ENROLMENT_SUMMARY_KEY + userId;
      int ttl =
          StringUtils.isNotBlank(
                  ProjectUtil.getConfigValue(JsonKey.USER_ENROLMENTS_RESPONSE_CACHE_TTL))
              ? Integer.parseInt(
                  ProjectUtil.getConfigValue(JsonKey.USER_ENROLMENTS_RESPONSE_CACHE_TTL))
              : 60;
      cacheUtil.set(key, mapper.writeValueAsString(response), ttl);
    } catch (Exception e) {
      logger.error(
          request.getRequestContext(),
          "Failed to cache user enrolment summary for user: " + userId,
          e);
    }
  }
}
